      ...
      <div className="item-list">
        { cart && cart.map((cartItem) => (
          <div className="item-card" key={cartItem.product._id}>
            <img className="item-image" src={cartItem.product.imgURL} alt={cartItem.product.name} />
            <h3 className="item-name">{cartItem.product.name}</h3>
            <div className="item-details">
              <button className="quantity-btn" onClick={() => dispatch(removeOneItem(cartItem.product))}>-</button>
              <span className="item-quantity">Quantity {cartItem.quantity}</span>
              <button className="quantity-btn" onClick={() => dispatch(addToCart(cartItem.product))}>+</button>
              <button className="remove-btn" onClick={() => dispatch(removeFromCart(cartItem.product))}>Remove</button>
            </div>
